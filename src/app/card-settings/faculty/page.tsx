...
import type { CardSettingsData, EmployeeData } from '@/lib/types';
...
import { Loader2, Save, SettingsIcon, Palette, FileText, Clock, Image as ImageIcon, ArrowLeft, Type, Briefcase } from 'lucide-react';
...
function CardSettingsContent() {
  ...
  useEffect(() => {
    async function loadSettings() {
      setIsLoading(true);
      try {
        const fetchedSettings = await getCardSettings('faculty');
        setSettings(fetchedSettings);
      } catch (error) {
        toast({ title: "Error Loading Settings", description: "Failed to fetch faculty card settings. Using defaults.", variant: "destructive" });
  ...
  const handleSaveSettings = async () => {
    setIsSaving(true);
    try {
      await saveCardSettings(settings, 'faculty');
      toast({ title: "Settings Saved", description: "Faculty card settings have been updated." });
    } catch (error) {
      toast({ title: "Error Saving Settings", description: "Could not save faculty settings.", variant: "destructive" });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleResetToDefaults = () => {
    setSettings(DEFAULT_CARD_SETTINGS);
    toast({ title: "Settings Reset", description: "Form reset to defaults. Click 'Save Settings' to persist."});
  };
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center min-h-[400px]">
        ...
        <p className="ml-2 text-muted-foreground">Loading Faculty Card Settings...</p>
      </div>
    );
  }
      ...
      <Card className="shadow-xl">
        <CardHeader>
          <CardTitle className="text-3xl font-bold text-primary flex items-center gap-2">
            <Briefcase size={32} /> Faculty Card Customization
          </CardTitle>
          <CardDescription>
            Customize the appearance for FACULTY ID cards. Card size: 54mm x 86mm.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid md:grid-cols-2 gap-8">
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div><Label htmlFor="frontDetailsFontSize">Details (Designation, Blood Grp, DOB)</Label><Input type="number" name="frontDetailsFontSize" value={settings.frontDetailsFontSize} onChange={handleNumberInputChange} /></div>
                <Separator />
                <h4 className="font-semibold text-md text-foreground">Back Side Font Sizes (px)</h4>
                <div><Label htmlFor="backDetailsFontSize">Top Details (ID, SEVARTH, Mobile)</Label><Input type="number" name="backDetailsFontSize" value={settings.backDetailsFontSize} onChange={handleNumberInputChange} /></div>
                ...
                <div><Label htmlFor="footerFontSize">Footer (Office Phone)</Label><Input type="number" name="footerFontSize" value={settings.footerFontSize} onChange={handleNumberInputChange} /></div>
              </CardContent>
            </Card>
          ...
          <div className="space-y-4 sticky top-8">
            <h3 className="text-xl font-semibold text-center text-foreground">Faculty ID Card Preview</h3>
            <Separator/>
            <EmployeeIdCard employee={mockEmployeeForPreview} settings={settings} showFlipButton={true}/>
          </div>
        </CardContent>
...
export default function FacultyCardSettingsPage() {
    return (
        <ProtectedRoute>
            <CardSettingsContent />
        </ProtectedRoute>

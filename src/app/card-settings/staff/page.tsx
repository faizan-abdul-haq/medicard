...
function CardSettingsContent() {
  ...
  useEffect(() => {
    async function loadSettings() {
      setIsLoading(true);
      try {
        const fetchedSettings = await getCardSettings('staff');
        setSettings(fetchedSettings);
      } catch (error) {
        toast({ title: "Error Loading Settings", description: "Failed to fetch staff card settings. Using defaults.", variant: "destructive" });
  ...
  const handleSaveSettings = async () => {
    setIsSaving(true);
    try {
      await saveCardSettings(settings, 'staff');
      toast({ title: "Settings Saved", description: "Staff card settings have been updated." });
    } catch (error) {
      toast({ title: "Error Saving Settings", description: "Could not save staff settings.", variant: "destructive" });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center min-h-[400px]">
        ...
        <p className="ml-2 text-muted-foreground">Loading Staff Card Settings...</p>
      </div>
    );
  }
      ...
      <Card className="shadow-xl">
        <CardHeader>
          <CardTitle className="text-3xl font-bold text-primary flex items-center gap-2">
            <Briefcase size={32} /> Staff Card Customization
          </CardTitle>
          <CardDescription>
            Customize the appearance for STAFF ID cards. Card size: 54mm x 86mm.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid md:grid-cols-2 gap-8">
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div><Label htmlFor="backDetailsFontSize">Top Details (ID, SEVARTH)</Label><Input type="number" name="backDetailsFontSize" value={settings.backDetailsFontSize} onChange={handleNumberInputChange} /></div>
                ...
                <div><Label htmlFor="footerFontSize">Footer (Phone)</Label><Input type="number" name="footerFontSize" value={settings.footerFontSize} onChange={handleNumberInputChange} /></div>
              </CardContent>
            </Card>
          ...
          <div className="space-y-4 sticky top-8">
            <h3 className="text-xl font-semibold text-center text-foreground">Staff ID Card Preview</h3>
            <Separator/>
...
export default function StaffCardSettingsPage() {
    return (
        <ProtectedRoute>
            <CardSettingsContent />
        </ProtectedRoute>
